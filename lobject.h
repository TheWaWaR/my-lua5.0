...
*/
/* (weet: union GCObject 在`lstate.h`中定义) */
...
*/
/* (weet:和CommonHeader没什么区别 ) */
typedef struct GCheader {
	// GCObject *next;        这个指针用来做什么? 让所有可GC的对象连成一个链表??
	// lu_byte tt;
	// lu_byte marked;        用来使用标记清除GC算法？
	CommonHeader;					
} GCheader;
...
/* (weet:TObject 是Tagged object的缩写, Lua类型系统的基础?)
   Object ==> type + value
 */
typedef struct lua_TObject {
  int tt;						/* (weet:tag, 用来标识该Object的类型) */
  Value value;
} TObject;


/* (weet: (o) is for TObject ) */
...
/* (weet:)
   1. 刚开始就设置obj的类型，难道Lua是动态类型的?
   2. 为什么不是先检查再赋值呢? (lua_assert 好像是个空函数)
 */
...
/* (weet:原来如此。。。。, memory alloc in heap not stack )  */
...
typedef union TString {
  ...
  struct {
    CommonHeader;
    lu_byte reserved;			/* (weet:unsigned char) */
    lu_hash hash;				/* (weet:unsigned int) */
    size_t len;
  } tsv;
} TString;
...
typedef struct Proto {
  CommonHeader;
  TObject *k;  /* constants used by the function (weet:该函数用到的常数列表) */
  Instruction *code;			/* (weet:字节码？) */
  struct Proto **p;  /* functions defined inside the function. (weet:为什么是指针的指针?) */ 
  int *lineinfo;  /* map from opcodes to source lines */
  struct LocVar *locvars;  /* information about local variables (weet:该函数的本地变量列表)*/
  TString **upvalues;  /* upvalue names */
  TString  *source;	   /* (weet:源代码？) */
  int sizeupvalues;
  int sizek;  /* size of `k' */
  int sizecode;
  int sizelineinfo;
  ...
  lu_byte numparams;			/* (weet:参数个数) */
  lu_byte is_vararg;			/* (weet:是否是可变参数) */
...
typedef struct LClosure {
  ClosureHeader;
  struct Proto *p;				/* (weet:指向Lua函数原型的指针) */
  TObject g;  /* global table for this closure */
  UpVal *upvals[1];
} LClosure;


/* 一个Closure可能是C Closure或者Lua Closure */
